...
import { useSocialLinks, SOCIAL_PLATFORMS, PLATFORM_META, type SocialLink } from '../hooks/useSocialLinks'
...
export const ProfileEditor: React.FC<{ onBack: () => void }> = ({ onBack }) => {
  ...
  useEffect(() => {
    ...
    for (const { platform, value } of links) map[platform] = value
    setDraft(map)
  }, [links])

  const handleSave = useCallback(async () => {
    const newLinks: SocialLink[] = Object.entries(draft)
      .filter(([, v]) => v.trim())
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      ...
      <div style={{
        ...
        >
          <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke={LC.textSub} strokeWidth="2.5" strokeLinecap="round">
            ...
            return (
              <div key={platform}>
                <label style={{
                  ...
                  fontSize: 11, fontWeight: 600, color: LC.textSub,
                  textTransform: 'uppercase', letterSpacing: '0.04em',
                  fontFamily: LC.font, marginBottom: 5,
                }}>
                  <span style={{ color: LC.orange }}>{PLATFORM_ICONS[platform]}</span>
                  {meta.label}
                </label>
                <input
      ...
      {/* Footer */}
      <div style={{ padding: '10px 14px', borderTop: `1px solid ${LC.border}`, flexShrink: 0 }}>
        {error && (
          <p style={{ fontSize: 11, color: '#ff6b6b', margin: '0 0 8px', fontFamily: LC.font }}>{error}</p>
        )}
        <button
          type="button"
          onClick={handleSave}
          ...
          style={{
            ...
            background: saved ? LC.teal : LC.orange,
